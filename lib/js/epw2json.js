...
//Need to review and finish this to get right data format
  epw.CircosHeatmap = function() {
      var month = this.getDataByField(1);
      var hour = this.getDataByField(3);
			var value = this.getDataByField(6);
			var heatmap_arrays = []
			...
			for (i = 0; i<23; i++) {
				...
				for (j = i; j<8759; i+24){
					...
					heatmap_array.push([month[j], count, count+1, value[j]]);
				}
				heatmap_arrays.push(heatmap_array);
      }
